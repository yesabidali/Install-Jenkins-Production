...
configure_ssl() {
  ...
  [[ "$ENABLE_SSL" != "yes" ]] && echo "Skipping SSL" && return

  read -p "Enter domain name (example: jenkins.example.com): " DOMAIN
  ...
  if [[ "$CERT_TYPE" == "letsencrypt" ]]; then
    ...
    if certbot certonly --nginx -d "$DOMAIN"; then
      SSL_CERT="/etc/letsencrypt/live/${DOMAIN}/fullchain.pem"
      SSL_KEY="/etc/letsencrypt/live/${DOMAIN}/privkey.pem"
      echo "Let’s Encrypt certificate issued"
    else
      echo "⚠️ Let’s Encrypt failed (DuckDNS/DNS issue). Using self-signed SSL."
  ...
  if [[ "$CERT_TYPE" == "selfsigned" ]]; then
    ...
    echo "⚠️ Self-signed SSL created"
  fi
...
EOF
  ...
  echo "Jenkins HTTPS URL: https://${DOMAIN}"
}
...
configure_sso() {
  ...
  [[ "$ENABLE_SSO" != "yes" ]] && echo "Skipping SSO" && return

  read -p "Choose SSO type (oidc/saml): " SSO_TYPE

  if [[ "$SSO_TYPE" == "oidc" ]]; then
    echo "OIDC selected"
    echo "Configure in Jenkins UI:"
    echo "Manage Jenkins → Security → OpenID Connect"

  elif [[ "$SSO_TYPE" == "saml" ]]; then
    echo "SAML selected"
    echo "Configure in Jenkins UI:"
    echo "Manage Jenkins → Security → SAML"
